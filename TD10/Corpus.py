...
class Corpus:
    ...
    def show(self, tri, n_docs=-1,):
        ...
        if tri == "123":
            ...
            docs = sorted(docs, key=lambda x: x.auteur)[:n_docs]
        elif tri == "type":
            docs = sorted(docs, key=lambda x: x.type)[:n_docs]
    ...
    def moteur_de_recherche(self, requete, vocab, mat_tfidf):
        ...
        for indice in indices_tries[:10]:
            ...
            """recherche_finale.append(f"Titre : {document.titre}\nAuteur : {document.auteur}\nSimilarité : {similarites[0, indice]:.4f}\nURL : {document.url}\n---")"""
            recherche_finale.append(f"{document.titre}\t\t")
            recherche_finale.append(f"Publié par {document.auteur}\n")
            recherche_finale.append(f"Similarité : {similarites[0, indice]:.4f}\t\t")
            recherche_finale.append(f"URL : {document.url}\n\n")

        return recherche_finale
